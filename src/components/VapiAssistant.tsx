...
const VapiAssistant: React.FC = () => {
    ...
    return (
        <div className="w-full max-w-2xl mx-auto mb-16">
            <div className={`relative p-8 rounded-[2rem] cosmic-glass glass-reflection border transition-all duration-500 overflow-hidden shadow-[0_0_50px_rgba(0,242,255,0.1)] ${status === 'connected' ? 'border-cyan-400/50 shadow-[0_0_80px_rgba(0,242,255,0.2)]' : 'border-white/10'}`}>

                {/* Background Glow Animation */}
                <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-cyan-400/20 blur-[80px] rounded-full transition-all duration-1000 ${status === 'connected' ? 'scale-150 opacity-80' : 'scale-50 opacity-0'}`}></div>

                <div className="relative z-10 flex flex-col items-center text-center gap-6">

                    {/* Status Badge */}
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-technical uppercase tracking-widest border transition-colors duration-300 ${status === 'connected' ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50' : 'bg-white/5 text-neutral-400 border-white/10'}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${status === 'connected' ? 'bg-cyan-400 animate-pulse' : status === 'connecting' ? 'bg-yellow-400 animate-bounce' : 'bg-neutral-500'}`}></span>
